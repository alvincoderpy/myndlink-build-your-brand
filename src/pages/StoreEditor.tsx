...
import { ArrowLeft, Save, Monitor, Tablet, Smartphone, Undo, Redo, Eye } from "lucide-react";
...
const StoreEditor = () => {
  ...
  return <div className="h-screen flex flex-col overflow-hidden bg-background">
      ...
      <div className="border-b bg-background px-4 py-2 flex items-center justify-between h-14 flex-shrink-0">
        ...
        <div className="hidden md:flex items-center gap-1">
          <div className="flex items-center gap-0.5 border rounded-md p-0.5">
            <Button variant={viewMode === "desktop" ? "secondary" : "ghost"} size="sm" className="h-8 w-8 p-0" onClick={() => setViewMode("desktop")}>
              <Monitor className="w-4 h-4" />
            </Button>
            <Button variant={viewMode === "tablet" ? "secondary" : "ghost"} size="sm" className="h-8 w-8 p-0" onClick={() => setViewMode("tablet")}>
              <Tablet className="w-4 h-4" />
            </Button>
            <Button variant={viewMode === "mobile" ? "secondary" : "ghost"} size="sm" className="h-8 w-8 p-0" onClick={() => setViewMode("mobile")}>
              <Smartphone className="w-4 h-4" />
            </Button>
          </div>

          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 ml-2" onClick={undo} disabled={!canUndo}>
        ...
        }} disabled={!store?.subdomain || !store?.is_published} className="h-9 hidden md:flex">
            <Eye className="w-4 h-4 mr-2" />
            Ver Loja
          </Button>
          
          {/* Mobile: Toggle Preview */}
          <Button size="sm" variant="outline" onClick={() => setShowPreview(!showPreview)} className="h-9 md:hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Config Panel */}
        <div className={cn("w-full lg:w-80 border-b lg:border-r lg:border-b-0 bg-background overflow-y-auto flex-shrink-0", showPreview && "hidden md:block")}>
        ...
        {/* Preview */}
        <div className={cn("flex-1 flex flex-col bg-muted/20 overflow-hidden", !showPreview && "hidden md:flex")}>
