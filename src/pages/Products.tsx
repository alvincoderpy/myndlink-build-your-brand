...
function SortableProductCard({
  ...
  return <div ref={setNodeRef} style={style} className="relative w-full">
      <Card className="p-3 h-[150px] flex items-center gap-3">
        ...
        <div className="absolute top-1.5 left-1.5 flex items-center gap-1.5 z-10">
          <div {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing p-0.5 hover:bg-muted rounded bg-background/80 backdrop-blur-sm">
            <GripVertical className="w-3.5 h-3.5 text-muted-foreground" />
          </div>
          <Badge variant="outline" className="font-mono text-xs bg-background/80 backdrop-blur-sm">
            #{index + 1}
          </Badge>
          {product.is_mock && <Badge variant="secondary" className="text-xs bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-100">
              Mock
            </Badge>}
        </div>
        ...
        <div className="flex-shrink-0 w-28 h-full">
          {product.image_url ? <img src={product.image_url} alt={product.name} className="w-full h-full object-cover rounded-md" /> : <div className="w-full h-full bg-muted rounded-md flex items-center justify-center">
        ...
        <div className="flex-1 flex flex-col h-full justify-between pt-6 pb-1">
          <div className="space-y-0.5">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {product.is_featured && <Badge className="bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-100 text-xs">
                  Destaque
                </Badge>}
              {product.is_new && <Badge className="bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-100 text-xs">
                  Novo
                </Badge>}
              {product.discount_percentage > 0 && <Badge className="bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-100 text-xs">
          ...
          <div className="flex gap-1.5">
            <Button variant="outline" size="sm" className="flex-1 h-7 text-xs" onClick={() => onEdit(product)}>
              <Edit className="w-3 h-3 mr-1" />
              Editar
            </Button>
            <Button variant="outline" size="sm" className="h-7 px-2 border-red-200 text-red-600 hover:bg-red-50 dark:border-red-900 dark:text-red-400 dark:hover:bg-red-950" onClick={() => onDelete(product.id)}>
...
const Products = () => {
    ...
    try {
      ...
      if (storeData) {
        const {
          data: productsData,
          error: productsError
        } = await supabase.from("products").select("*").eq("store_id", storeData.id).order("display_order", {
          ascending: true,
          nullsFirst: false
        }).order("created_at", {
          ascending: false
  ...
  return loading ? <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {[1, 2, 3].map(i => <Card key={i} className="p-3 h-[150px]">
            <Skeleton className="w-full h-full" />
          </Card>)}
      </div>
    </div> : !store ? <div className="flex items-center justify-center min-h-[350px]">
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
                ...
                <FormField control={form.control} name="discount_percentage" render={({
                field
              }) => <FormItem>
                      <FormLabel>Desconto (%)</FormLabel>
                      <FormControl>
                        <Input type="number" min="0" max="100" {...field} onChange={e => field.onChange(parseFloat(e.target.value) || 0)} value={field.value || 0} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>} />
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {form.watch("image_url") && <img src={form.watch("image_url")} alt="Preview" className="w-24 h-24 object-cover rounded" />}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <FormField control={form.control} name="is_new" render={({
                  field
                }) => <FormItem className="flex items-center justify-between rounded-lg border p-3">
                        <div>
      ...
      {products.length === 0 ? <Card className="p-8 text-center">
          ...
          <SortableContext items={sortedProducts.map(p => p.id)} strategy={rectSortingStrategy}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {sortedProducts.map((product, index) => <SortableProductCard key={product.id} product={product} index={index} onEdit={handleEdit} onDelete={handleDelete} />)}
            </div>
          </SortableContext>
        </DndContext>}
